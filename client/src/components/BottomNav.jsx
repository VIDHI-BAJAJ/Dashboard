...
function NavIcon({ index, active }) {
  ...
  const color = active ? "text-[#004f98]" : "text-[#004f98]";
...
export function BottomNav() {
  return (
    <nav
      className="lg:hidden fixed inset-x-0 bottom-0 h-16 bg-white border-t border-white/10 flex items-center justify-around px-2 pb-[env(safe-area-inset-bottom)] z-40"
    >
        ...
        <NavLink
          ...
          className={({ isActive }) =>
            `flex flex-col items-center justify-center text-[11px] font-medium tracking-wide transition-all duration-200 ${
              isActive ? "text-[#004f98]" : "text-[#004f98] hover:text-[#004f98]"
            }`
          }
        >
          {({ isActive }) => (
