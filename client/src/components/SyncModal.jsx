...
export default function SyncModal({
  ...
  const [publishingListings, setPublishingListings] = useState(false);
  const [publishedSuccess, setPublishedSuccess] = useState(false);
  const [publishError, setPublishError] = useState(null); // ← NEW: store error details
  const [activationLoading, setActivationLoading] = useState(false);
  const [pollActive, setPollActive] = useState(initialPortalStatus === "pending");
    ...
    if (activePortal === "realestate") {
      ...
      try {
        ...
        const res = await fetch(`${API_URL}/api/portal/setup-sync`, {
          method: "POST",
          headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` },
          body: JSON.stringify({ agentId: agentId.trim(), clientId: clientId.trim(), clientSecret: clientSecret.trim() }),
        });
        const data = await res.json();
        if (data.success) {
          setPollActive(true);
  ...
  // ─────────────────────────────────────────────────────────────
  // PUBLISH LISTINGS — with proper error handling
  // ─────────────────────────────────────────────────────────────
  const handlePublishListings = async () => {
    if (selectedListings.length === 0) {
      alert("Please select at least one listing.");
    ...
    try {
      ...
      } else if (data.published?.length > 0 && data.failed?.length > 0) {
        // Partial success — some published, some failed
        setPublishError({
          type: "partial",
          published: data.published.length,
          failed: data.failed,
          message: `${data.published.length} published, ${data.failed.length} failed`,
        });

      } else if (data.failed?.length > 0) {
        // All failed — show REA's actual error messages
        setPublishError({
          type: "error",
          failed: data.failed,
  ...
  const renderCredentialsForm = () => {
    ...
    return (
      <div className="bg-gray-50 border border-gray-200 rounded-xl p-5 mb-4">
        <p className="text-sm font-semibold text-gray-700 mb-4">
          Enter your REA credentials
          <span className="ml-2 text-xs font-normal text-gray-400">Find in REA Partner Portal</span>
        </p>
        <div className="mb-3">
          <label className="block text-xs font-medium text-gray-600 mb-1">REA Agent ID <span className="text-red-400">*</span></label>
          <input type="text" value={agentId} onChange={(e) => setAgentId(e.target.value)}
            placeholder="e.g. XNWTEL"
            className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#004f98]/30 focus:border-[#004f98]" />
        </div>
        <div className="mb-3">
          <label className="block text-xs font-medium text-gray-600 mb-1">Client ID <span className="text-red-400">*</span></label>
          <input type="text" value={clientId} onChange={(e) => setClientId(e.target.value)}
            placeholder="e.g. 3539d925-bb88-48c1-..."
            className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#004f98]/30 focus:border-[#004f98]" />
        </div>
        <div className="mb-1">
          <label className="block text-xs font-medium text-gray-600 mb-1">Client Secret <span className="text-red-400">*</span></label>
          <div className="relative">
        ...
        {credError && <p className="text-xs text-red-500 mt-2">{credError}</p>}
        <p className="text-xs text-gray-400 mt-3">🔒 Credentials encrypted and stored securely.</p>
      </div>
    );
  };
      ...
      <div className="bg-white w-[750px] max-h-[90vh] overflow-y-auto rounded-2xl shadow-2xl">
              ...
              {portals.map((portal) => (
                <div key={portal.id} className="w-[280px] bg-white rounded-2xl shadow-md border border-gray-100 overflow-hidden">
                  ...
                  <div className="p-5">
                    ...
                    <button onClick={() => { setActivePortal(portal.id); setStep(1); }}
                      className="w-full bg-[#004f98] text-white py-2 rounded-xl hover:bg-[#003b75] transition">
                      {portal.id === "realestate" && portalStatus === "connected" ? "Manage Portal" : "Activate Portal"}
                    </button>
                  </div>
        ...
        {step === 2 && (
          <div className="p-8">
            ...
            {/* ── Error State ── */}
            {!publishedSuccess && publishError && (
              <div className="py-6">
                <div className={`rounded-xl p-5 mb-6 ${publishError.type === "partial" ? "bg-yellow-50 border border-yellow-200" : "bg-red-50 border border-red-200"}`}>
                  <div className="flex items-center gap-3 mb-3">
                    <span className="text-xl">{publishError.type === "partial" ? "⚠️" : "❌"}</span>
                    <p className={`font-semibold text-sm ${publishError.type === "partial" ? "text-yellow-700" : "text-red-700"}`}>
                      {publishError.message}
                    </p>
                  </div>
                  {publishError.failed?.length > 0 && (
                    <div className="space-y-2 mt-3">
                      <p className="text-xs font-medium text-gray-600">Issues to fix:</p>
                      {publishError.failed.map((f, i) => (
                        <div key={i} className="bg-white rounded-lg px-3 py-2 text-xs text-red-600 border border-red-100">
                          <span className="font-medium">{f.headline || f.listingId}:</span> {f.error}
                        </div>
                      ))}
                    </div>
                  )}
                ...
                <div className="flex gap-3">
                  ...
                  <button onClick={handlePublishListings} disabled={publishingListings}
                    className="flex-1 bg-[#004f98] hover:bg-[#003b75] disabled:bg-gray-300 text-white px-4 py-2 rounded-lg text-sm transition">
                    {publishingListings ? "Retrying..." : "🔄 Retry"}
                  </button>
                </div>
              </div>
            )}

            {/* ── Select Listings State ── */}
            {!publishedSuccess && !publishError && (
              <>
                <div className="flex justify-between items-center mb-6">
                  <div>
                ...
                <div className="flex justify-between items-center pt-4 border-t border-gray-100">
                  <button onClick={() => setStep(0)} className="text-gray-400 hover:text-gray-600 text-sm">← Back</button>
                  <button onClick={handlePublishListings}
                    disabled={selectedListings.length === 0 || publishingListings}
                    className="bg-green-600 hover:bg-green-700 disabled:bg-gray-300 disabled:cursor-not-allowed text-white px-6 py-2 rounded-lg text-sm font-medium transition flex items-center gap-2">
                    {publishingListings ? (
                      <><svg className="animate-spin w-4 h-4" fill="none" viewBox="0 0 24 24">
