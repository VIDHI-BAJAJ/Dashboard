...
const publishListings = async (req, res) => {
    ...
    for (const listing of listings) {
      try {
        // ── Step 1: Convert JSON → REAXML ──
        const xmlPayload = generateREAXML(listing, agentId);

        console.log("📄 Generated XML:\n", xmlPayload);

        console.log(`📤 Uploading listing ${listing._id} as XML to REA...`);

        // ── Step 2: POST XML to REA upload endpoint ──
        const response = await axios.post(
          ...
          xmlPayload,
          {
            headers: {
              Authorization:  `Bearer ${accessToken}`,
              "Content-Type": "application/xml", // ← XML not JSON
              Accept:         "application/json",
            },
          }
        );

        // ── Step 3: Mark as published in MongoDB ──
        await Listing.findByIdAndUpdate(listing._id, {
          "rea_portal.status":       "published",
          "rea_portal.connected_at": new Date(),
        });
